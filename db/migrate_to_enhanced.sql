-- Migration script to upgrade existing campus_db to enhanced version
-- Run this script on your existing database to add all enhancements
...
-- ============================================================================
-- STEP 1: Add new tables for enhanced functionality
-- ============================================================================

-- Check if enhanced tables already exist before creating
...
-- ============================================================================
-- STEP 2: Add performance indexes (only if they don't exist)
-- ============================================================================
...
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS
     ...
     AND index_name = 'idx_bookings_room_time') = 0,
    'CREATE INDEX idx_bookings_room_time ON bookings(room_id, start_time, end_time)',
    'SELECT "Index idx_bookings_room_time already exists"'
));
...
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS
     WHERE table_schema = 'campus_db'
     AND table_name = 'bookings'
     AND index_name = 'idx_bookings_user_status') = 0,
    'CREATE INDEX idx_bookings_user_status ON bookings(user_id, status)',
    'SELECT "Index idx_bookings_user_status already exists"'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS
     WHERE table_schema = 'campus_db'
     AND table_name = 'bookings'
     AND index_name = 'idx_bookings_status_time') = 0,
    'CREATE INDEX idx_bookings_status_time ON bookings(status, start_time)',
    'SELECT "Index idx_bookings_status_time already exists"'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS
     WHERE table_schema = 'campus_db'
     AND table_name = 'bookings'
     AND index_name = 'idx_bookings_availability') = 0,
    'CREATE INDEX idx_bookings_availability ON bookings(room_id, status, start_time, end_time)',
    'SELECT "Index idx_bookings_availability already exists"'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
-- ============================================================================
-- STEP 3: Create enhanced triggers (drop and recreate for safety)
-- ============================================================================
...
-- Equipment auto-tracking trigger
...
-- ============================================================================
-- STEP 4: Create enhanced stored procedures
-- ============================================================================

DELIMITER //

-- Enhanced conflict checking procedure (update existing)
DROP PROCEDURE IF EXISTS check_conflict//
...
-- New enhanced booking procedure with locking
...
-- Equipment maintenance locking procedure
...
-- ============================================================================
-- STEP 6: Migrate existing data to audit tables
-- ============================================================================

-- Create initial audit entries for existing bookings
INSERT INTO audit_bookings (booking_id, action_type, new_data, changed_by)
SELECT
    id,
    'INSERT',
    ...
    user_id
FROM bookings
WHERE id NOT IN (SELECT DISTINCT booking_id FROM audit_bookings WHERE action_type = 'INSERT');

-- Create equipment usage entries for approved bookings
INSERT INTO equipment_usage (equipment_id, booking_id, notes)
SELECT DISTINCT e.id, b.id, CONCAT('Migrated usage record for ', r.name)
FROM bookings b
JOIN rooms r ON b.room_id = r.id
JOIN equipment e ON r.id = e.room_id
WHERE b.status = 'approved'
AND NOT EXISTS (
    SELECT 1 FROM equipment_usage eu
    WHERE eu.equipment_id = e.id AND eu.booking_id = b.id
);
...
SELECT 'Enhanced tables created:' as message;
SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES
WHERE TABLE_SCHEMA = 'campus_db'
AND TABLE_NAME IN ('audit_bookings', 'audit_users', 'equipment_usage', 'user_sessions', 'password_history', 'login_attempts');

SELECT 'New indexes created:' as message;
SELECT DISTINCT INDEX_NAME FROM INFORMATION_SCHEMA.STATISTICS
WHERE TABLE_SCHEMA = 'campus_db'
AND INDEX_NAME LIKE 'idx_%';

SELECT 'Stored procedures available:' as message;
